...
if($user_name == true) {
    ...
    if($row = mysqli_fetch_assoc($show_query_data)) {
        ...
        $sql_validate_data = "SELECT * FROM `unit`";
        $show_query_data = mysqli_query($con, $sql_validate_data);
        while($unit_row = mysqli_fetch_assoc($show_query_data)) {
            $unit_sno = $unit_row["unit_sno"]+1;
        }
    } else {
        header("location : ../");
    }
        ...
        <div class="user-form">
            <form action="php_forms_post/add_unit_post.php" method="post">
                ...
                <div class="group">
                    ...
                    <input type="text" name="sno" id="sno" disabled value="<?php echo $unit_sno; ?>">
